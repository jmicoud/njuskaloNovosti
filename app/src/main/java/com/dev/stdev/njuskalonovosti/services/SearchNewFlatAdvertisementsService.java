	...
	public void parseAllValues(String valStr, String upit) {
		...
		if (!tableSearch.isSearchTableEmpty()) //==false)
		{
			...
			if (!isNs) //==false) //this is new search
			{
				...
				tableSearch.addSearch(prNew);

				fl.setIsNewFlat("1"); //this isnew apartment becouse it is new search so value is 1

				tableFlat.addFlat(fl, Integer.toString(newGeneralId)); //add apartment to database, novistanovi table
			...
			} else //isNs=true
			{
				...
				List<FlatAdvertismentClass> flLs = tableFlat.getAllApartments(tempGenId);
				...
				{
					fl.setIsNewFlat("1");
					tableFlat.addFlat(fl, tempGenId);
				} else {
					fl.setIsNewFlat("0");
